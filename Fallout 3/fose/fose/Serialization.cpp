...
void SetPreLoadCallback(PluginHandle plugin, FOSESerializationInterface::EventCallback callback)
{
	ASSERT(plugin);
	ASSERT(plugin <= g_pluginManager.GetNumPlugins());
...
void InternalSetPreLoadCallback(PluginHandle plugin, FOSESerializationInterface::EventCallback callback)
{
	if(plugin >= s_pluginCallbacks.size())
		s_pluginCallbacks.resize(plugin + 1);
...
void HandleSaveGame(const char * path)
{
	...
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_SaveGame, (void*)path, strlen(path), NULL);
...
{
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_PreLoadGame, (void*)path, strlen(path), NULL);
...
void HandlePostLoadGame(bool bLoadSucceeded)
{
	...
	// inform plugins
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_PostLoadGame, (void*)bLoadSucceeded, 1, NULL);

	// handle internal stuff
	Core_PostLoadCallback(bLoadSucceeded);
}

void HandleLoadGame(const char * path, FOSESerializationInterface::EventCallback PluginCallbacks::* callback)
{
	// pass file path to plugins registered as listeners
	if (!s_preloading) {
		PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_LoadGame, (void*)path, strlen(path), NULL);
	}
	...
	else
	{
		try
		{
			...
			FOSESerializationInterface::EventCallback curCallback = NULL;
...
void HandleDeleteGame(const char * path)
{
	...
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_DeleteGame, (void*)savePath.c_str(), strlen(savePath.c_str()), NULL);
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_DeleteGameName, (void*)saveName.c_str(), strlen(saveName.c_str()), NULL);
...
void HandleRenameGame(const char * oldPath, const char * newPath)
{
	...
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_RenameGame, (void*)oldSavePath.c_str(), strlen(oldSavePath.c_str()), NULL);
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_RenameGameName, (void*)oldSavePath.c_str(), strlen(oldSavePath.c_str()), NULL);
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_RenameNewGame, (void*)newSavePath.c_str(), strlen(newSavePath.c_str()), NULL);
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_RenameNewGameName, (void*)newSavePath.c_str(), strlen(newSavePath.c_str()), NULL);
...
{
	PluginManager::Dispatch_Message(0, FOSEMessagingInterface::kMessage_NewGame, NULL, 0, NULL);
	// iterate through plugins
	for(UInt32 i = 0; i < s_pluginCallbacks.size(); i++)
	{
